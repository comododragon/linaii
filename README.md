...
This dual mode is simply done by calling Lina twice for each design point. All arguments can be the same within calls, except for `--mma-mode`. On the first execution, it shall be `--mma-mode=gen`. On the second exeution, it shall be `--mma-mode=use`.
