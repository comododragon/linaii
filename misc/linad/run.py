...
def explore(console, options, experiment, kernels):
	...
	for k in kernels:
		...
		try:
			...
			# Open daemons and set them up
			for i in range(noOfJobs):
				console.setProgress(i, 0, "Opening linad (pipe id {})...".format(i), "Exploring {} (job {})".format(k, i + 1))
				pipePath = "/tmp/linad.pipe.{}".format(i)

				# Check if a pipe file exists. If positive, close it
				if os.path.exists(pipePath):
					with open(pipePath, "w") as pipeF:
						pipeF.write("q\n")

				# Wait few seconds and see if file disappears. If not, linad is frozen...
				time.sleep(2)
				if os.path.exists(pipePath):
					raise RuntimeError("Daemon with pipe ID {} is frozen. Solve it before running run.py".format(i))

				# Now open linad
				subprocess.run(
					["linad", "{}".format(i)], env=modEnv, check=True,
					stderr=subprocess.DEVNULL if "yes" == options["SILENT"][1] else subprocess.STDOUT
				)

				# Wait some time until pipe is established
				attempts = 5
				while True:
					if 0 == attempts:
						raise RuntimeError("Linad error: pipe never came up on \"{}\"".format(pipePath))
					else:
						if os.path.exists(pipePath):
							break
						attempts -= 1
				...
				console.setProgress(i, 0, "Setting linad (pipe id {})...".format(i), "Exploring {} (job {})".format(k, i + 1))
				with open(pipePath, "w") as pipeF:
					dynamicTraceFile = os.path.join(os.getcwd(), "workspace", experiment, k, "base", "dynamic_trace.gz")
					pipeF.write("d{:03}{}\n".format(len(dynamicTraceFile), dynamicTraceFile))
					# For job 0, we also include the memory trace
					if 0 == i:
						memoryTraceFile = os.path.join(os.getcwd(), "workspace", experiment, k, "base", "mem_trace_short.bin")
						pipeF.write("l{:03}{}\n".format(len(memoryTraceFile), memoryTraceFile))
			...
			# Close daemons
			for i in range(noOfJobs):
				#console.setProgress(i, 100, "Closing linad (pipe id {})...".format(i), "Exploring {} (job {})".format(k, i + 1))
				pipePath = "/tmp/linad.pipe.{}".format(i)
